...
# Add the current directory to sys.path to ensure 'dahell_backend' and 'core' can be found
...
def analyze():
    ...
    for c in top_concepts:
        concept_name = c['taxonomy_concept']
        count = c['count']
    ...
    for cluster in saturated_clusters:
        # Use cluster.concept_name if available, else try to get it from representative product's concept
        ...
        output.append(f"   - Cluster {cluster.cluster_id} ({name}): {cluster.total_competitors} competitors")
